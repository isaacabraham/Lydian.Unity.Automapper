...
let ``Single mapping that already exists in the container throws an exception``() =
    ...
    raises<DuplicateMappingException>(<@ validateMapping defaultConfig container defaultBehaviors (typeof<obj>, typeof<int>) @>)
...
let ``Single mapping that already exists in the container does not throw an exception when the type is a multimap``() =
    ...
    validateMapping { defaultConfig with MultimapTypes = [typeof<obj>] } container defaultBehaviors (typeof<obj>, typeof<int>)
...
let ``Existing named mapping with the same name but for a different type does not throw an exception``() =
    ...
    validateMapping { defaultConfig with ExplicitNamedMappings = [typeof<DateTimeOffset>, "TESTNAME"] } container defaultBehaviors (typeof<DateTime>, typeof<DateTimeOffset>)
...
let ``Existing named mapping with the same name for the same type throws an exception``() =
    ...
    raises<DuplicateMappingException>(<@ validateMapping config container MappingBehaviors.MultimapByDefault (typeof<obj>, typeof<DateTimeOffset>) @>)
