...
let ``Multimapped types are correctly identified``() = 
    let config = 
        { defaultConfig with MultimapTypes = 
                                 [ typeof<string>
...
let ``AndDoNotMapFor correctly merges blacklist``() = 
    let config = (AutomapperConfig { defaultConfig with DoNotMapTypes = [ typeof<string> ] }).AndDoNotMapFor typeof<int>
    test <@ Seq.length config.Data.DoNotMapTypes = 2 @>

[<Fact>]
let ``AndUseNamedMappingsFor correctly merges list``() = 
    let config = 
        (AutomapperConfig { defaultConfig with ExplicitNamedMappings = [ typeof<string>, "TEST" ] })
...
let ``AndUseMultimappingFor correctly merges list``() = 
    let config = 
        (AutomapperConfig { defaultConfig with MultimapTypes = [ typeof<string> ] }).AndUseMultimappingFor typeof<int>
    test <@ Seq.length config.Data.MultimapTypes = 2 @>

[<Fact>]
let ``AndUsePolicyInjectionFor correctly merges list``() = 
    let config = 
        (AutomapperConfig { defaultConfig with PolicyInjectionTypes = [ typeof<string> ] })
...
let ``MapAsSingleton uses Container Controlled Lifetime Manager``() = 
    let config = AutomapperConfig.Create().AndMapAsSingleton(typeof<int>)
    test <@ config.Data.CustomLifetimeManagerTypes.Head
            |> snd = typeof<ContainerControlledLifetimeManager> @>
