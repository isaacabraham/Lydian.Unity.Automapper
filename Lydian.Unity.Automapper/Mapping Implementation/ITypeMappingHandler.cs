...
namespace Lydian.Unity.Automapper
{
	...
	internal interface ITypeMappingHandler
	{
		...
		/// </summary>
		/// <param name="target">The cotainer to use to perform registrations.</param>
		...
		/// <returns></returns>
		IEnumerable<ContainerRegistration> PerformRegistrations(IUnityContainer target, IEnumerable<TypeMapping> typeMappings, MappingBehaviors mappingBehaviors, AutomapperConfig configurationDetails);
	}
}
